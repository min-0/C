...
void EnglishScore(int score[]){ //시험 결과
	srand((unsigned int)time(NULL)); //난수 생성을 위한 선언

	for (int i = 0; i < 100; ++i) //100명의 학생 성적 넣기
	{
		score[i] = rand() % 101; //랜덤한 큰 수에서 101로 나눈 나머지 값으로 난수 생성
	}
}
void grade(int score[]){ //성적 내림차순 정렬
	int temp; //비교할 때 필요한 복사할 변수
	for (int i = 0; i < 100; i++){ //100개 성적 정렬
		for (int k = 0; k < i - 1; k++){ //버블소트
